...
controller.compareRecruitToRecruit = function(req, res) {
if (!req.query.recruit || !req.query.comparisonRecruit) {
    ...
    var getComparisonRecruitTimes = new Promise(function(f, r) {
      Recruit.findOne({"name": req.query.comparisonRecruit}, 'times height')
             .populate('times')
             .exec(function(err, r) {
                if (err) {
                  r(err);
    ...
    Promise.all([getRecruitTimes, getComparisonRecruitTimes])
      .catch(function(err) {
        res.status(500).send(err);
      })
      .then(function(results) {
        var recruitTimes = results[0].times;
        var reference = results[1].times;
        if (!recruitTimes) {
          return res.status(400).send({"error": "This recruit does not exist."});
        }
        if (reference.length == 0) {
          return res.status(400).send({"error": "This recruit used for comparison does not exist."});
        }
